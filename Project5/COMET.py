...
class COMET:
    ...
    def __init__(self):
        ...
        self.routers = {
            'rhea': ('127.0.0.1', 2, 4503),
            'queeg': ('127.0.0.1', 9, 4501),
            'glados': ('127.0.0.1', 16, 4504)}
    ...
    def comet_send(self):
        ...
        :return: None
        """
        comet_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # socket creation
        data = self.get_packet()
        address1 = (self.routers['queeg'][0], self.routers['queeg'][-1])  # address to send to
        address2 = (self.routers['rhea'][0], self.routers['rhea'][-1])  # address to send to
        comet_socket.sendto(data.encode(), address1)  # sending the data to the destination
        time.sleep(2)
        comet_socket.sendto(data.encode(), address2)
        comet_socket.close()  # closing the socket

    def comet_receive(self):
        """
        Receive function
        :return: None
        """
...
def main():
    comet = COMET()
    p1 = Process(target=comet.comet_send)
    p1.start()
    count = 0
