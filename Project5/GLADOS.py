...
class GLADOS:
    ...
    def __init__(self):
        ...
        self.routers = {
            'queeg': ('127.0.0.1', 1, 4501),
            'rhea': ('127.0.0.1', 1, 4503),
            'comet': ('127.0.0.1', 16, 4502)}
    ...
    def split_horizon_poison_reverse(self):
        ...
        for each_router in self.routing_table:
            ...
            for each in self.routing_table:
                ...
                # else we set the cost to 16 and the next hop to none
                else:
                    self.routing_table[each][1] = 16
                    self.routing_table[each][2] = None
    ...
    def glados_send(self):
        """
        Send function
        :return: None
        """
        glados_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # socket creation
        data = self.get_packet()
        address1 = (self.routers['rhea'][0], self.routers['rhea'][-1])  # address to send to
        address2 = (self.routers['queeg'][0], self.routers['queeg'][-1])  # address to send to
        glados_socket.sendto(data.encode(), address1)  # sending the data to the destination
        time.sleep(2)
        glados_socket.sendto(data.encode(), address2)
        glados_socket.close()

    def glados_receive(self):
        """
        Receive function
        :return: None
        """
        ...
        for each in self.neighbour_routing_table:
            self.neighbour_routing_table[each] = None
        for _ in range(2):
            response, addr = self.socket.recvfrom(1024)  # decoding the response
            packet = response.decode().split('\n')  # the router from which data has arrived
            node = packet[0]
            routing_tbl = dict()
            for i in range(1, len(packet)):
                line = packet[i].split()
...
def main():
    glados = GLADOS()
    p1 = Process(target=glados.glados_send)
    p1.start()
    count = 0
    for _ in range(1000):
        glados.glados_receive()
